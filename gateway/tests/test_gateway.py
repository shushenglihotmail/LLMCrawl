...
with patch.dict("sys.modules", {"openai": Mock(), "httpx": Mock(), "fastapi": Mock()}):
...
class TestToolHandler:
    ...
    async def test_tool_call_structure(self, tool_handler):
        ...
        mock_tool_call = {
            ...
            "function": {
                ...
                "arguments": json.dumps({"query": "test query", "freshness_days": 7}),
            },
        }

        # Mock the internal methods
        with patch.object(
            tool_handler, "_handle_crawl_and_refresh", new_callable=AsyncMock
        ) as mock_crawl:
            mock_crawl.return_value = {"test": "result"}

            result = await tool_handler.handle_tool_call(
                mock_tool_call, "test_request_id"
            )
    ...
    async def test_crawl_and_refresh_pipeline(self, tool_handler):
        ...
        with (
            patch.object(
                tool_handler, "_call_crawler", new_callable=AsyncMock
            ) as mock_crawler,
            patch.object(
                tool_handler, "_call_indexer_index", new_callable=AsyncMock
            ) as mock_indexer,
            patch.object(
                tool_handler, "_call_indexer_retrieve", new_callable=AsyncMock
            ) as mock_retrieve,
        ):
