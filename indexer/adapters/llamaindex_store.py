...
class LlamaIndexStore:
    ...
    async def index_documents(self, documents: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            indexed_count = await self.vector_store.add_documents(all_chunks, embeddings)
    ...
    async def retrieve_documents(
        ...
        try:
            ...
            for result in boosted_results:
                url = result["url"]
                if url not in url_groups or result["boosted_score"] > url_groups[url]["boosted_score"]:
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                embedding_healthy = len(test_embedding) > 0 and len(test_embedding[0]) > 0
            except:
                embedding_healthy = False
            
            return {
                "status": "healthy" if vector_health.get("status") == "healthy" and embedding_healthy else "degraded",
