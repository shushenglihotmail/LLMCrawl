...
async def crawl_endpoint(request: CrawlRequest, req: Request):
    ...
    try:
        ...
        if request.seed_urls:
            request.seed_urls = await robots_checker.filter_allowed_urls(request.seed_urls)
        ...
        if len(documents) < max(1, request.max_results // 2):
            logger.info("Attempting Playwright fallback")
            renderer = await get_playwright_renderer()
            
            # If we have seed URLs, try rendering them
            fallback_urls = request.seed_urls[:3]  # Limit fallback attempts
            if fallback_urls:
                try:
                    ...
                    logger.info(f"Playwright fallback added {len(rendered_docs)} documents")
        ...
        logger.info(f"Crawl completed {request_id}: {len(filtered_docs)} documents in {duration_ms:.1f}ms")
...
async def health_check():
    ...
    try:
        ...
        # Check Firecrawl
        if firecrawl_client:
            health_status["components"]["firecrawl"] = await firecrawl_client.health_check()
        else:
