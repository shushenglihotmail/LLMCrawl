...
**Environment:**
- Python 3.10, 3.11, 3.12 (all supported; CI builds on 3.12)
- Node.js - Used by Firecrawl and Playwright containers (not directly by project code)

**Package Manager:**
- pip with setuptools (build backend)
- Lockfile: Not present (uses version ranges in `pyproject.toml` and `deploy/requirements/*.txt`)
...
**Critical (Gateway - `deploy/requirements/gateway.txt`):**
...
**Critical (Crawler - `deploy/requirements/crawler.txt`):**
...
**Critical (Indexer - `deploy/requirements/indexer.txt`):**
...
**Critical (Memory Service - `deploy/requirements/memory_service.txt`):**
...
**Environment:**
- Environment variables loaded from `deploy/.env` (existence noted, contents not read)
- `LLM_PROVIDER`: azure | openai | anthropic - Primary LLM provider
...
**Linter:** flake8 (max-line-length 88, extends-ignore E203,E221,E231,E713)
...
**Container Images:**
- `deploy/Dockerfile.crawler` - Crawler service
- `deploy/Dockerfile.indexer` - Indexer service
- `deploy/Dockerfile.mcp_server` - Azure DevOps MCP server (also has `mcp_servers/azure_devops_mcp_server/Dockerfile`)
- `deploy/Dockerfile.demo` - Demo web client
- `services/memory_service/Dockerfile` - Memory service (built and pushed to GHCR via CI)
...
- `hichat` -> `clients.hichat.main:main` - Web client launcher
- `llmcrawl` -> `llmcrawl_cli.main:main` - Main CLI (deploy, bridge management, status)
...
- Windows recommended (scripts in PowerShell: `scripts/start-services.ps1`, `scripts/stop-services.ps1`)
